...
- `probe.url`（string，probe 存在时必填，http/https）。
- `probe.expectedStatuses`（[Int]，可选，缺省 `[200]`）。
- 旧 config 无 `probe` 字段照常解码；阶段 1 冻结字段全部不变。

#### app 执行器语义

- start：spawn `command`（首元素可执行路径，参数原样；不经 shell）；stdout/stderr 追加写入 `~/Library/Logs/TunnelPad/<id>.log`；写 pidfile `~/Library/Application Support/TunnelPad/run/<id>.pid`。
- status：子进程存活 → running(pid)；否则已停止。
- stop：SIGTERM → 最多 5s → SIGKILL；删除 pidfile。
- keepAlive=true：非手动退出（意外退出/外部 kill）→ 延迟 `throttleInterval` 秒自动重启；手动 stop 置 manualStop 标志不再拉起。
- 退出 app：正常退出路径在 applicationShouldTerminate 后台块内对全部 app 子进程 SIGTERM→5s→KILL；SIGTERM/SIGINT 信号路径读 pidfile kill（app 进程内存不可依赖）；崩溃边界与 launchd 相同（子进程残留、下次启动可按 pidfile 清理）。

#### 探针语义

- refresh 时对配置了 probe 的隧道异步执行 HTTP GET：绕过系统代理（空 connectionProxyDictionary），超时 3s，不跟随需要凭据的响应。
- 三态：满足（status ∈ expectedStatuses）/ 不满足（其他 2xx-5xx）/ 失败（连接错误、超时、非 HTTP）。
- 探针只影响展示（行内徽标与文字），不影响隧道进程启停。

#### 日志查看

- launchd 与 app 执行器统一使用 `~/Library/Logs/TunnelPad/<id>.log`。
- 日志 sheet 显示文件末 500 行；2s 自动刷新开关（默认开）；提供手动刷新与复制全部。
...
| 阶段 | 阶段 2 |
| 结论 | 通过（达到待实施标准） |
| 证据 | 独立复核轮次核对：阶段 2 准入摘要七字段齐备；样本矩阵 9 行含命令/预期/失败判定/输出位置（grep 核对 9/9）；Step 0 双基线落实（ModelPad `ModelProcessManager.swift` 已读取、阶段 0/1 探测期望在案）；失败/回滚边界明确；`plan-governance-cli check .` 与 `--strict-readiness` 均通过 |
