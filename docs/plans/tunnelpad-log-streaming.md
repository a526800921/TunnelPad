...
本计划独立处理 TunnelPad 的日志采集、内存缓存、追加事件和面板生命周期，不并入隧道探针健康恢复计划。阶段 0 可以先完成基线和准入设计；Rust Core 迁移阶段 5 已完成并关闭，本计划阶段 1 仍需自身阶段 0 准入，不与 Rust Core owner 并行修改共享执行器。当前实现范围先按 `launchd` owner 设计，未来 `app` 执行器重新开发后再补充对应日志采集范围。
...
|---|---|
| 准入状态 | 设计中 |
| Step 0 | 基线类型已确定为“行为迁移现状快照 + 隔离最小复现”；源码、日志路由、Rust owner 和当前工作树边界已核验并落盘，事件驱动无轮询的可执行最小复现尚待补齐 |
...
| 当前阻塞项 | Rust Core 阶段 5 已完成并关闭；本计划阶段 0 的隔离最小复现、事件时序 fixture 和独立准入尚未完成，且当前工作树存在备注阶段 1 的侧栏共享文件改动；这些阻塞阶段 1 实施，但不阻塞本阶段继续补齐文档基线 |
...
尚未完成的 Step 0 证据是“可控子进程追加一行日志后，当前 UI 只有下一次 Timer 才能看到；面板关闭期间无法通过事件订阅实时交付，切换隧道需要重新读取快照；日志超过 2000 行时当前实现没有安全裁剪”的隔离最小复现。复现必须使用 fake executor、隔离日志文件和可控输出，不得修改真实用户日志文件；完成后追加到本节和阶段证据文档。当前已完成源码、日志路由、Rust owner 和并行工作树边界核验，见[阶段 0 基线证据](../data-quality/tunnelpad-log-streaming-stage0-20260831.md)。
...
| 阶段 | 阶段 0 |
| 结论 | 尚未达到待实施标准 |
| 证据 | Step 0 现状最小复现、事件采集 fixture 和阶段 0 样本矩阵尚待执行；Rust Core 迁移阶段 5 已完成并关闭，当前仅等待本计划自身证据 |
| 复核者 | 尚未指定 |
...
| 阶段 1 实施时 Rust Core 的日志 owner | Rust Core 迁移阶段 5 已确认 Rust 单一 owner；实施前禁止 Swift/Rust 双重采集同一 `launchd` 隧道 | 是（阶段 1 实施） | 本计划阶段 0 待冻结 |
