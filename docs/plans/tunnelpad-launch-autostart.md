...
| 阶段 3 | 启动自动恢复与真实验收 | 阶段 1、阶段 2 完成 | 专项测试 + 真实登录/重启验收 + Release/治理门禁 | 已完成 |
...
| 2026-09-12 | 构建/替换 | build_app.sh 全流程通过，替换运行中的旧 App，恢复机制实机生效（app.log 候选 0 符合预期） | 阶段 1–3 实施证据 | 完成（自验） | ZCode（实施者） |
| 2026-09-12 | 用户验收 | 用户确认真实验收通过 | 本计划用户可观察验收表 | 通过；计划关闭 | 用户 |
...
|---|---|---|---|---|
| 登录自启 | 打包 .app 运行中 | 菜单栏勾选"登录时自动启动"；系统设置登录项可见 TunnelPad | 重启或注销后登录，TunnelPad 自动出现在菜单栏 | 用户确认（2026-09-12） |
| 隧道自动恢复 | 隧道 A `autoStart=true`，B 默认关闭；两者均停止 | 重启电脑（或退出后重新打开打包 .app） | A 自动进入运行态、探针/状态正常；B 保持停止；列表状态与启动日志可查 | 用户确认（2026-09-12） |
| 恢复失败可见 | A 指向需 ECS 同步的 SSH 且网络断开 | 启动 App 触发恢复 | A 保持停止、B 不受影响，无重试风暴；失败原因写入 `~/Library/Logs/TunnelPad/app.log`（含成功/跳过明细） | 用户确认（2026-09-12，app.log 已见恢复明细） |
| 旧配置兼容 | 不含 `autoStart` 的 v1 config.json | 新版 App 加载并正常增删改启停 | 行为与升级前一致；保存后新字段按表单值写入 | 阶段 1 差分与单测证据；真实升级观察（2026-09-12 替换运行中 App） |
...
| 问题 | 推荐方案 | 是否阻塞当前阶段 | 状态 |
|---|---|---|---|
| 真实登录/重启用户验收 | 已由用户于 2026-09-12 确认通过 | 否 | 已完成 |
| 范围外改动归属 | 工作树所有者确认随本计划一并提交（4797bec 后的工作树改动已入 94fab94） | 否 | 已完成 |
| 范围外改动归属 | 工作树所有者确认随本计划一并提交（4797bec 后的工作树改动已入 94fab94） | 否 | 已完成 |
