...
ECS 动态 SSH 计划已完成“TunnelPad 手动启动/重启前同步公网 IP”，但该前置只发生在 `TunnelManager` 的显式入口。若隧道运行期间本机公网 IPv4 发生变化，现有 `launchd` `KeepAlive` 的直接重连不会自动经过 ECS 同步；连接中断后可能使用旧安全组来源反复重连失败。该运行中 IP 变化、规则同步和自动恢复边界现纳入本稳定性计划阶段 0 设计，具体触发时机与恢复 owner 待冻结。
...
| 健康恢复协调器在自动重连前调用既有 ECS 同步命令 | 复用既有双端点校验、受管规则和安全组写入边界；需要处理与 `KeepAlive`、手动停止和退避任务的竞态 | **阶段 0 候选优先方案，待 fixture 冻结** |
